...
description: "介绍 gid (Git Identity Manager)，一个用 Rust 编写的命令行工具，通过规则引擎和 Git Hooks 实现多 Git 身份的自动化管理。"
categories:
- 技术实践
- 工具使用
tags:
...
在日常开发中，我需要在工作项目与个人项目之间频繁切换。手动修改 Git 用户名和邮箱不仅繁琐，还容易出错；一旦忘记切换，错误身份会被写入历史，后续清理成本很高。

为了解决这个问题，我开发了 `gid`（Git Identity Manager），一个用 Rust 编写的命令行工具，用于自动化多身份管理。本文聚焦“问题-方案-落地-边界”的完整链路。
...
1. 首次配置身份和规则
2. 为常用仓库安装 hook
3. 进入仓库后执行 `gid auto` 自动应用身份
4. 提交前通过 `git config user.name` / `git config user.email` 快速确认

这样可以把“忘记切换身份”的风险降到最低。
...
- 如果你只有单一身份或极少跨仓切换，`gid` 可能过重，简单脚本即可满足需求。
- 对历史记录的修复仍需配合 `git filter-repo` 等工具完成，需谨慎评估协作影响。
