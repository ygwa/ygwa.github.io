...
简而言之，**SDD 让 AI 从“聪明的盲人摸象者”，变成“有图纸的建筑工人”。**

<!--more-->

本文将对三款主流 SDD 工具做结构化对比，并给出选择建议。

## Spec Kit（GitHub）

### 工具定位

[Spec Kit](https://speckit.org/) 是 GitHub 推出的开源 SDD 工具包，与 GitHub Copilot 等生态深度融合（GitHub Star 数据截至 2026-03-10）。它通过斜杠命令与 AI 代理交互，执行严格的**四阶段工作流**，适合在现有编辑器（VSCode、JetBrains 等）中集成使用。
...
[OpenSpec](https://openspec.dev/) 强调维护单一、统一的规范文档作为系统设计的权威参考（GitHub Star 数据截至 2026-03-10）。它支持 20+ 主流 AI 编码工具（Cursor、Claude Code 等），采用「意图驱动」的工作流。
...
- 如果团队尚未形成规范化协作习惯，SDD 的收益会被执行成本抵消。
- 工具链引入后需要持续维护规范文档，否则容易“形式化”而失去效果。
