...
在多人协作的项目中，误提交敏感信息（密钥、数据库密码等）几乎不可避免。若尚未推送远端，可用 `git reset --soft` 快速修正；一旦推送，单纯删除文件已无法抹除历史记录，因为 Git 会永久保留提交内容，安全风险会持续存在。

本文从一个实际案例出发，介绍如何使用 `git filter-repo` 清理敏感信息，并补充适用场景与注意事项。
...
那么如何在不丢失历史记录的前提下，从 Git 仓库中删除敏感信息呢？
...
```sh
# 包管理软件安装
apt install git-filter-repo # for linux
...
## 我的思考
通过 `git filter-repo` 我们可以非常灵活地修改 Git 的历史记录，其强大的功能给了我们很多操作空间。当然这也不是没有代价，在对于其原理的分析中我们可以看到，修改历史提交对于仓库本身的影响还是比较大的，且会关联影响到很多历史记录。在多人协作或者采用 **monorepo** 的时候，这种操作往往会产生较大的影响。因此对于历史的修改，不应该是一种常态。迫不得已时，我们更多时候需要尊重历史。如果我们希望避免修改的发生，可以从以下几个方面进行优化：
...
- `git filter-repo` 会重写历史，必须在团队统一窗口内执行并强制同步。
- 对公开仓库或多方协作仓库，优先评估“撤销泄露影响”的成本，再决定是否重写历史。
