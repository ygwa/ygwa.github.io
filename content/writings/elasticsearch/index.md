...
Elasticsearch 是一个高可用的搜索引擎。最近在项目中由于客户基础设施成熟度不足，需要 Ops 进行 Elasticsearch 的高可用部署。我借此系统梳理了集群架构与数据存储机制，本文做一次结构化总结。
...
Elasticsearch 使用基于多数派的集群协调机制进行主节点选举：
...
- 生产环境需要结合实际硬件与负载进行容量规划，本文仅覆盖最小可靠架构与关键机制。
- 具体参数（如 JVM、分片数、ILM 策略）需要通过压测与观测数据调优。
